 ...
 */
public class ThreadGPSAsyncTask extends AsyncTask<Void, Bundle, Void> {
    ...
    public static boolean salveCoordenadas = false;
    public static Coordenada coordenada;

    public ThreadGPSAsyncTask(Context context, GoogleMap m) {
        ...
        while (isProcess) {
            ...
            if (gps_enabled) {
                ...
                if (gps_loc == null) {
                    ...
                    info.putString("action", "atualizar_lm_gps");
            ...
            if (network_enabled) {
                ...
                if (net_loc == null) {
                    ...
                    Bundle info = new Bundle();
                    info.putString("action", "atualizar_lm_network");
    ...
    protected void onProgressUpdate(Bundle... values) {
        ...
        if (values[0].get("action") != null) {

            // Caso getLastKnownLocation tenha retornado null é necessário atualizar a lm
            if (values[0].get("action").toString().equals("atualizar_lm_gps")) {

                Toast.makeText(context, "Atualizando Location Manager GPS", Toast.LENGTH_SHORT).show();
                lm.requestLocationUpdates(LocationManager.GPS_PROVIDER, 0, 0, locationListenerGps);
                return;
            }

            if (values[0].get("action").toString().equals("atualizar_lm_network")) {

                Toast.makeText(context, "Atualizando Location Manager Network", Toast.LENGTH_SHORT).show();
        ...
        if (salveCoordenadas) {
            ...
            coordenada.setLatitude(values[0].get("latitude").toString());
            coordenada.setLongitude(values[0].get("longitude").toString());
            coordenada.setTempo(values[0].get("time").toString());

            CoordenadaDao coordenadaDao = new CoordenadaDao(context);
            ContentValues contentValues = new ContentValues();
            contentValues.put("tempo", coordenada.getTempo());
            contentValues.put("longitude", coordenada.getLongitude());
            contentValues.put("latitude", coordenada.getLatitude());
            contentValues.put("id_rota", coordenada.getIdRota());

            Log.i("CadastrandoCoordenadas", "id:" + coordenada.getIdRota() + "|longitude:" + coordenada.getLongitude() + "|latitude:" + coordenada.getLatitude() + "|tempo:" + coordenada.getTempo() + " salve:" + coordenadaDao.cadastrarCoordenada(contentValues));
    ...
    public void setarPonto(LatLng PONTO) {
        ...
        BitmapDescriptor icon = BitmapDescriptorFactory.fromResource(R.drawable.pirates);
    ...
    public static void setLongi(double longi) {
        ThreadGPSAsyncTask.longi = longi;
    }
